# This file provides the main code to the automation for OrangeHRM for PIMTC3
...
class Pimtest3:
    ...
    def login(self):
            ...
            self.driver.find_element(by=By.XPATH, value=locators_pim1.newadd().empname_input_box).send_keys(data_pim1.Employeeinfo().EmployeeName)
            searchbtn = self.driver.find_element(by=By.XPATH, value=locators_pim1.newadd().searchbtn_box).click()

            delete_button = self.driver.find_element(by=By.XPATH, value=locators_pim1.newadd().delete_box).click()
            wait = WebDriverWait(self.driver, 30)               
            cnfrmdlt= self.driver.find_element(by=By.XPATH, value=locators_pim1.newadd().dltbutn).click()
            ...
            action.click(on_element=pimbtn).perform() 
            action.click(on_element=searchbtn).perform()
            action.click(on_element=delete_button).perform()    
            action.click(on_element=cnfrmdlt).perform()  

    def shutdown(self):
        self.driver.quit() 
        

y = Pimtest3(data_pim1.Data_login1().url)
y.login()
y.shutdown()
